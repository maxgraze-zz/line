...
const Tooltip = ({
  ...
  const drawContent = React.useCallback(
    x => {
      ...
      tooltipContent.select('.contentTitle').text(xScale.invert(x));
    },
    [xScale, margin, width],
  );
  ...
  const onChangePosition = React.useCallback((d, i, isVisible) => {
    ...
    d3.selectAll('.performanceItemMarketValue')
      .filter((td, tIndex) => tIndex === i)
      .text(d.value && !isVisible ? 'No data' : d.value);
  ...
  const followPoints = React.useCallback(() => {
    ...
    d3.select(ref.current)
      ...
      .attr('transform', (cur, i) => {
        console.log(data[i]);
        const index = bisectDate(data[i].value, xDate, 1);
        const d0 = data[i].value[index - 1];
        const d1 = data[i].value[index];
    ...
    <g ref={ref} opacity={0} {...props}>
      ...
      <g className='tooltipContent'>
        ...
        <g className='content' transform='translate(4,32)'>
          {data.map(({ type }, i) => (
            <g key={type} transform={`translate(6,${22 * i})`}>
      ...
      {data.map(({ type }) => (
        <circle className='tooltipLinePoint' r={6} key={type} opacity={0} />
      ))}
    </g>
  );
};
